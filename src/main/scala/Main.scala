...
// Fastest reader config for production
  val productionReaderConfig: ReaderConfig = ReaderConfig
    .withThrowReaderExceptionWithStackTrace(
      false
    ) // no stack traces (default, keep it)
    .withAppendHexDumpToParseException(
      false
    ) // skip hex dump = faster error paths
    .withCheckForEndOfInput(
      false
    ) // skip trailing-bytes check if input is trusted
    .withPreferredBufSize(
      65536
    ) // 64KB: fewer reallocations for typical messages
    .withPreferredCharBufSize(
      8192
    ) // 8KB: fewer reallocations for string-heavy payloads
...
// Fastest codec generation settings
  given CodecMakerConfig = CodecMakerConfig
    ...
    .withRequireDiscriminatorFirst(
      true
    ) // ADT discriminator must be first field (default)
    .withInlineOneValueClasses(
      true
    ) // inline single-arg wrapper types = zero boxing overhead
