...
final case class KeyManagementConfig(
  ...
  def validate: IO[Config.Error, Unit] = ZIO
    .attempt {
      require(
    ...
    .mapError(e => Config.Error.InvalidData(Chunk.empty, e.getMessage))
}

object KeyManagementConfig {
  val config: Config[KeyManagementConfig] = deriveConfig[KeyManagementConfig]

  val layer: ZLayer[Any, Config.Error, KeyManagementConfig] =
...
object CachingConfig {
  val config: Config[CachingConfig] = deriveConfig[CachingConfig]

  val layer: ZLayer[Any, Config.Error, CachingConfig] =
    ZLayer.fromZIO(ZIO.config(config))
}
...
object IdentityServerConfig {
  val config: Config[IdentityServerConfig] = deriveConfig[IdentityServerConfig]

  val layer: ZLayer[Any, Config.Error, IdentityServerConfig] =
    ZLayer.fromZIO(ZIO.config(config))
}
...
class DefaultKeyManager(
  ...
  // Validation is now an IO effect, call it during construction
  // For backward compat, we run it unsafely here. Better would be to use a ZIO entry point.
  zio.Unsafe.unsafe { implicit unsafe =>
    zio.Runtime.default.unsafe.run(options.keyManagement.validate).getOrThrow()
  }
