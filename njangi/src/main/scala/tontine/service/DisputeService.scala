        ...
        new DisputeService {
          ...
          override def raiseDispute(
            ...
            for {
              ...
              _ <- ZIO
                .fail(
                  AppError.Validation(
                    "Contribution does not belong to stated circle"
                  )
                )
              ...
              _ <- ZIO
                .fail(
                  AppError.Validation(
                    "Contribution does not belong to stated member"
                  )
                )
          ...
          override def notifyMember(
            ...
            for {
              ...
              _ <- ZIO
                .fail(
                  AppError.Validation(
                    s"Dispute is already ${d.status}; cannot notify"
                  )
                )
          ...
          override def resolveDispute(
            ...
            for {
              ...
              _ <- ZIO
                .fail(
                  AppError.Validation("Dispute is not in a resolvable state")
                )
                .when(
                ...
                case DisputeStatus.PenaltyApplied =>
                  for {
                    ...
                    penalty = Penalty(
                      ...
                      circleId = d.circleId,
                      amount = penaltyAmt,
                      reason = req.resolutionNote.getOrElse(
          ...
          override def penaltiesByMember(
              memberId: MemberId
          ): UIO[List[Penalty]] =
