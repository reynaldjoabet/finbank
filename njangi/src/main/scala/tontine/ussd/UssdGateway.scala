 ...
 *     `ContributionService`, `ScoreService`, and `CircleService` that the
 *     REST API uses.
 ...
 *
 * USSD network integration:
 *   Expose a `POST /ussd/callback` endpoint that passes the body parameters
 *   (`sessionId`, `phoneNumber`, `text`) to `handle(...)`.  Compatible with
 *   Africa's Talking USSD API format.
 *
...
trait UssdGateway {
   ...
   * Handle one USSD hop.
   *
   * @param sessionId   Unique session identifier from the USSD aggregator.
   * @param phoneE164   Caller's phone number in E.164 format.
   * @param input       The digit(s) the user entered (empty string on first hop).
   * @return USSD response text + whether the session is terminal.
   */
  def handle(
      sessionId: String,
      phoneE164: String,
...
object UssdGateway {
  ...
  val live: ZLayer[
    ...
    ZLayer.fromZIO {
      ...
      } yield new UssdGatewayLive(contributions, circles, scores, members, sessions)
    }
  ...
  private final class UssdGatewayLive(
      ...
      session.currentMenu match {
        ...
        case UssdMenuId.Main =>
          input match {
            ...
            case _ =>
              ZIO.succeed(UssdResponse("Invalid option. " + mainMenu.text, isFinal = false))
          }
          ...
          input match {
            ...
            case "0" =>
              transition(session.sessionId, UssdMenuId.Main) *>
                ZIO.succeed(mainMenu)
            case _ =>
              ZIO.succeed(UssdResponse("Invalid option.\n" + loanMenu.text, isFinal = false))
          }
        ...
        case UssdMenuId.LoanAmount =>
          ...
          maybeAmt match {
            case None =>
              ZIO.succeed(UssdResponse("CON Invalid amount. Enter a number:\n0. Back", isFinal = false))
            case Some(amt) if amt < 1000 || amt > 50000 =>
              ZIO.succeed(UssdResponse("CON Amount out of range (1000–50000 XAF).\n0. Back", isFinal = false))
            case Some(amt) =>
          ...
          input match {
            ...
            case "2" | "0" =>
              transition(session.sessionId, UssdMenuId.Main) *>
                ZIO.succeed(mainMenu)
            case _ =>
              ZIO.succeed(UssdResponse("CON Press 1 to confirm, 2 to cancel.", isFinal = false))
          }
    ...
    private def updateContext(sessionId: String, key: String, value: String): UIO[Unit] =
      sessions.update { m =>
        m.get(sessionId) match {
          case Some(s) => m + (sessionId -> s.copy(context = s.context + (key -> value)))
          case None    => m
        }
      }
    ...
    private def endSession(sessionId: String): UIO[UssdResponse] =
      sessions.update(_ - sessionId) *>
        ZIO.succeed(UssdResponse("END Thank you for using Finbank.", isFinal = true))
