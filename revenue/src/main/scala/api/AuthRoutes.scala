...
object AuthRoutes {
  ...
  val routes: Routes[Env, Nothing] =
    Routes(
      ...
      Method.POST / "api" / "v1" / "auth" / "refresh" -> handler { (req: Request) =>
        (for {
      ...
      Method.POST / "api" / "v1" / "auth" / "logout" -> handler { (req: Request) =>
        (for {
          body <- JsonSupport.decode[LogoutRequest](req)
          svc <- ZIO.service[AuthService]
          _ <- svc.logout(body)
        } yield Response.status(Status.NoContent)).catchAll(e => ZIO.succeed(JsonSupport.errorJson(e)))
      },

      Method.POST / "api" / "v1" / "auth" / "password" / "forgot" -> handler { (req: Request) =>
        (for {
          body <- JsonSupport.decode[ForgotPasswordRequest](req)
          svc <- ZIO.service[AuthService]
          _ <- svc.forgotPassword(body)
        } yield Response.status(Status.Accepted)).catchAll(e => ZIO.succeed(JsonSupport.errorJson(e)))
      },

      Method.POST / "api" / "v1" / "auth" / "password" / "reset" -> handler { (req: Request) =>
        (for {
      ...
      Method.GET / "api" / "v1" / "auth" / "me" -> handler { (req: Request) =>
        ...
        } yield JsonSupport.okJson(p)).catchAll(e => ZIO.succeed(JsonSupport.errorJson(e)))
      }
    )
}
