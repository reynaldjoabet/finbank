...
<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pacs.008.001.10">
  <FIToFICstmrCdtTrf>
    ...
    <CdtTrfTxInf>
      ...
      <IntrBkSttlmAmt Ccy="${m.amount.currency.code}">${formatMinor(m.amount)}</IntrBkSttlmAmt>
      <Dbtr>
  ...
  /** Convert minor-unit Long to a decimal string with correct scale.
   *  XAF / XOF have 0 decimal places (1 XAF = 1 minor unit).
   *  All others assumed 2 decimal places.
   */
...
/**
 * ZIO service that builds and validates pacs.008 messages before submission
 * to a regional clearing system.
 */
...
object Iso20022Service {
  ...
  val live: ULayer[Iso20022Service] = ZLayer.succeed {
    new Iso20022Service {
      ...
      override def submitToClearing(xml: String): IO[Iso20022Error, ClearingAck] =
        for {
