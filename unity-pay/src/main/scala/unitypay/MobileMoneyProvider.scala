...
/**
 * Routes a `MoMoTransferRequest` to the correct `MobileMoneyProvider`
 * based on the `provider` field.  This is the single entry-point for all
 * pan-African mobile money payments inside `unity-pay`.
 *
 * Usage:
 * {{{
