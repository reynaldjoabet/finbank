 ...
 *   - Only the SHA-256 hash of the token is persisted (same pattern as
 *     refresh tokens in `TokenHash`).
    ...
    ZLayer.fromZIO(
      Ref.make(Map.empty[ConsentId, Consent]).map { store =>
        new ConsentService {
          ...
          override def create(req: CreateConsentRequest): IO[AppError, ConsentTokenResponse] =
            for {
              _ <- ZIO
                .fail(AppError.Validation("ttlSeconds must be > 0"))
                .when(req.ttlSeconds <= 0)
              _ <- ZIO
                .fail(AppError.Validation(s"ttlSeconds must be ≤ $MaxTtlSeconds (90 days)"))
          ...
          override def authorise(consentId: ConsentId, userId: UUID): IO[AppError, Consent] =
            for {
              c <- getById(consentId)
              _ <- ZIO
                .fail(AppError.Forbidden("You are not the resource owner of this consent"))
              ...
              _ <- ZIO.logInfo(s"[Consent] Authorised id=${consentId.value} user=$userId")
            } yield updated
          ...
          override def resolve(
            ...
            for {
              ...
              c <- ZIO
                ...
                .orElseFail(AppError.Unauthorized("Invalid or unknown consent token"))
              _ <- ZIO
          ...
          override def revoke(consentId: ConsentId, userId: UUID): IO[AppError, Unit] =
            for {
              c   <- getById(consentId)
              _   <- ZIO
                .fail(AppError.Forbidden("You are not the resource owner of this consent"))
                .when(c.userId != userId)
              now <- Clock.instant
              updated = c.copy(status = ConsentStatus.Revoked, revokedAt = Some(now))
              _ <- store.update(_ + (consentId -> updated))
              _ <- ZIO.logInfo(s"[Consent] Revoked id=${consentId.value} user=$userId")
            } yield ()
          ...
          private def getById(id: ConsentId): IO[AppError, Consent] =
            store.get.flatMap { m =>
              ZIO.fromOption(m.get(id)).orElseFail(AppError.NotFound(s"Consent ${id.value} not found"))
            }
        }
      }
    )
