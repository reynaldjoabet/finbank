...
import com.migrantbank.api.HttpUtils.*
 ...
 *
 * Endpoints:
 *   POST   /v1/consents               — TPP creates a consent grant
 *   POST   /v1/consents/{id}/authorise — User authorises (bank-side redirect)
 *   DELETE /v1/consents/{id}          — User revokes a consent
 *   GET    /v1/consents               — List consents for the authenticated user
 */
    ...
    Routes(
      ...
      Method.POST / "v1" / "consents" ->
        handler { (req: Request) =>
          ...
          } yield jsonResponse(result, Status.Created))
            .merge
        },

      // ── POST /v1/consents/{id}/authorise ────────────────────────────────
      Method.POST / "v1" / "consents" / zio.http.codec.PathCodec.uuid("consentId") / "authorise" ->
        handler { (consentId: UUID, req: Request) =>
          (for {
            ...
            consent <- ZIO
              ...
              .mapError(mapConsentError)
          } yield jsonResponse(consent))
            .merge
        },

      // ── DELETE /v1/consents/{id} ─────────────────────────────────────────
      Method.DELETE / "v1" / "consents" / zio.http.codec.PathCodec.uuid("consentId") ->
        handler { (consentId: UUID, req: Request) =>
          (for {
            userIdStr <- ZIO
              .fromOption(req.headers.get("X-User-Id"))
