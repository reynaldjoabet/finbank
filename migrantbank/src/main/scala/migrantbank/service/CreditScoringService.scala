...
/** Parameters associated with a credit band.
  ...
  * @param collateralReq
  *   Whether physical collateral is required.
  */
...
object CreditScoringService {
  ...
  private def classifyBand(
    ...
    val tontinePoints =
      tontineScore
        .map(s => if s >= 80 then 40 else if s >= 50 then 25 else 10)
        .getOrElse(0)
    val momoPoints =
      (if txCount >= 20 then 30
       else if txCount >= 10 then 20
       else if txCount >= 3 then 10
       else 0) +
        (if volumeMinor >= 1_000_00L then 30
         else if volumeMinor >= 500_00L then 20
         else if volumeMinor >= 100_00L then 10
         else 0)
