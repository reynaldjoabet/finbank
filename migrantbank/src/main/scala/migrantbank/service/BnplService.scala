      ...
      for {
        ...
        override def requestPlan(
          ...
          for {
            _ <- ZIO
              .fail(
                AppError.Validation("instalmentCount must be between 1 and 12")
              )
            ...
            card <- existingCards
              ...
              case None    =>
                ZIO.fail(
                  AppError.NotFound(
                    s"No active virtual card found for user $userId; issue one first"
                  )
                )
            }
        ...
        override def getPlan(planId: UUID): IO[AppError, BnplPlan] =
          plans.get.flatMap { m =>
            ZIO
              .fromOption(m.get(planId))
              .orElseFail(AppError.NotFound(s"BNPL plan $planId not found"))
          }

        override def listByUser(userId: UUID): IO[AppError, List[BnplPlan]] =
          plans.get.map(_.values.filter(_.userId == userId).toList)

        override def instalments(
            planId: UUID
        ): IO[AppError, List[BnplInstalment]] =
        ...
        override def markInstalmentPaid(
          ...
          for {
            ...
            inst <- ZIO
              ...
              .orElseFail(
                AppError.NotFound(
                  s"Instalment $sequence not found in plan $planId"
                )
              )
            _ <- ZIO
            ...
            _ <- ZIO.when(newList.forall(_.paid)) {
              plans.update(m =>
                m.get(planId) match {
                  case Some(p) =>
                    m + (planId -> p.copy(status = BnplPlanStatus.FullyRepaid))
                  case None => m
                }
              )
            }
