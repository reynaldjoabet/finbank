package com.migrantbank.webhook
 ...
 *
 * Each event carries enough context for a third-party webhook consumer to
 * act without making additional API calls.
 */
 ...
 *
 * @param eventTypes  Which event types to deliver.  Empty set = all events.
 * @param secret      HMAC-SHA256 signing secret for payload verification.
 *                    Stored hashed; the raw secret is shown only at creation time.
 */
 ...
 *
 * Partners register endpoint URLs via `subscribe`.  Every time `emit` is
 * called (by a service that just completed a mutation), the webhook service
 * fans out to all matching subscribers with exponential-backoff retries.
 *
 * Payload format:
 * {{{
 *   POST <targetUrl>
...
trait WebhookService {
  ...
  def deliveries(subscriptionId: SubscriptionId): IO[AppError, List[WebhookDelivery]]
}
...
object WebhookService {
  ...
  private final class WebhookServiceLive(
    ...
    override def unsubscribe(id: SubscriptionId, tppClientId: String): IO[AppError, Unit] =
      for {
        all <- subs.get
        sub <- ZIO
